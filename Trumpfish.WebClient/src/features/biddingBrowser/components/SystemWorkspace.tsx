...
import { BidEditorPanel } from './BidEditorPanel';
...
interface SystemWorkspaceProps {
  /**
   * System narzucony przez widok, w którym edytor stoi. Podany - edytor nie pozwala wybrać innego, bo wybiera się go tam,
   * gdzie ten sam wybór znaczy coś jeszcze. `undefined` znaczy „edytor stoi sam" i wybiera system u siebie.
   */
  systemId?: string | null;
  /** Ścieżka, w obrębie której nawigacja nie jest zatrzymywana pytaniem o niezapisany system - w widoku analizy zmiana rozdania to wciąż ten sam widok. */
  withinPath?: string;
  /** Mówi gospodarzowi, czy system ma niezapisane zmiany: to gospodarz pyta, zanim podmieni system pod edytorem. */
  onDirtyChange?: (dirty: boolean) => void;
}

/**
 * Drzewo odzywek, edytor jednej z nich, pasek komend i wynik walidacji - całe narzędzie do systemu, w jednej kolumnie.
 */
/*
 * Komponent, a nie strona, bo stoi w dwóch miejscach: jako własne narzędzie pod `/tools/bidding-browser` i jako lewa
 * kolumna widoku analizy. Wszystko, czym te dwa miejsca się różnią, wchodzi trzema propsami wyżej; reszta jest wspólna,
 * co do skrótu klawiszowego.
 */
export function SystemWorkspace({ systemId: imposedId, withinPath, onDirtyChange }: SystemWorkspaceProps) {
  const { user } = useAuth();
  ...
  const embedded = imposedId !== undefined;
  ...
  /*
   * System narzucony z zewnątrz, wczytywany raz na każdy nowy identyfikator.
   *
   * Pytany o ostatnio uczciony identyfikator, a nie o ten, który jest w stanie: w edytorze można założyć nowy system,
   * który jeszcze nie ma identyfikatora, i porównanie ze stanem wczytałoby wtedy na niego z powrotem ten narzucony.
   *
   * Sprzątanie zdejmuje pamięć o uczcionym identyfikatorze, bo zdarza się przed każdym powtórnym wejściem tutaj - przy
   * zmianie systemu, przy odmontowaniu i przy podwójnym wywołaniu efektów w trybie ścisłym. Bez tego pierwsze pobranie
   * było unieważniane, a drugie już się nie zaczynało: edytor zostawał przy pustym systemie.
   */
  const honoured = useRef<string | null>(null);
  useEffect(() => {
    if (imposedId === undefined || imposedId === null || honoured.current === imposedId) {
      return;
    }

    honoured.current = imposedId;

    let cancelled = false;
    getBiddingSystem(imposedId).then(
      (system) => { if (!cancelled) { dispatch({ kind: 'loadSystem', system: normalizeSystem(system), systemId: imposedId }); } },
      (reason) => { if (!cancelled) { setError(describe(reason)); } },
    );
  ...
  // Gospodarz pyta o porzucenie zmian, zanim podmieni system, więc musi wiedzieć, czy jest o co pytać.
  useEffect(() => {
  ...
  const requestedId = embedded ? null : searchParams.get('system');
  useEffect(() => {
    if (requestedId === null) {
      return;
    }
   ...
   *
   * `withinPath` jest wyjątkiem dla widoku, który pod edytorem zmienia własny adres: w analizie przejście na inne rozdanie
   * jest wciąż tym samym widokiem i tym samym otwartym systemem, więc nie ma o co pytać.
   */
  const blocker = useBlocker(({ currentLocation, nextLocation }) => {
    ...
    return withinPath === undefined
      ? currentLocation.pathname !== nextLocation.pathname
      : !nextLocation.pathname.startsWith(withinPath);
  });
  ...
  return (
    <div className={embedded ? 'bidding-browser embedded' : 'bidding-browser'}>
